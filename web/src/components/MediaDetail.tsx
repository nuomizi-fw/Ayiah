...
import { getMediaItem, refreshMetadata } from "../api/library";
...
const MediaDetail: Component<MediaDetailProps> = (props) => {
    const [refreshing, setRefreshing] = createSignal(false);

    const { data, loading, error, send } = useRequest(() =>
        getMediaItem(props.itemId),
    );
    ...
    const handleRefresh = async () => {
        setRefreshing(true);
        try {
            await refreshMetadata(props.itemId).send();
            await send();
        } catch (err) {
            ...
            <Show when={error()}>
                <div class="max-w-4xl mx-auto">
                    <button
                        ...
                        class="flex items-center gap-2 text-neutral-400 hover:text-white mb-4"
                    >
